...
export const Tile = ({ x, y, parties, locationType }: TileProperties) => {
  return (
    <Group>
      ...
      {parties && parties[0] && parties[0].owner === PartyOwner.Enemy && (
        <Circle radius={14} fill="black" x={x} y={y} />
      )}
      {parties && parties[1] && parties[1].owner === PartyOwner.Player && (
        <Circle radius={14} fill="blue" x={x + 4} y={y + 4} />
      )}
      {parties && parties[1] && parties[1].owner === PartyOwner.Enemy && (
        <Circle radius={14} fill="black" x={x + 4} y={y + 4} />
      )}
    </Group>
  );
};
